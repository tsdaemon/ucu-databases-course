...
Note, that table `students_assignments` has a composite primary key, which consist  of 
two columns. Therefore REST API routes for `students_assignments` are slightly different:
instead of a single `id` argument they should accept `student_id` and `assignment_id`.
