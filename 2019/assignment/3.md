...
REST API should reuse standard HTTP codes with its own meaning:

| Case | Code |
|---|---|
| Resource not found (incorrect id) | 404 |
| Invalid data provided (validation failed) | 422 |
| Resource created | 201 |

Code from range `5..` means that application contains an error. For example, unhandled
...
All validation rules required for this assignment can be infered from column types in 
...
| GET | `/students_assignments/<int:id>` |
| POST | `/students_assignments/` |
| PUT | `/students_assignments/` |
| DELETE | `/students_assignments/<int:id>` |
| *Student summary* |
| GET | `/student_summary/` |
